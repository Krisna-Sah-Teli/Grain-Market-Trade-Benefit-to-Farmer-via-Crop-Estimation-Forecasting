...
def predictcrop():
        ...
        print("Model Score " + str(model.score(inputs,target)))
        ...
        ph=6.42
        rain=224.59
        k=model.predict([[temp,hum,ph,rain]])
        print(k)
        for i in range(0,12):
            if(k==targetarray[i]):
                index=i
        ...
        crops1=['paddy','wheat','moong','bajra','maize','masoor','jute','cotton','sugarcane','copra','urad','arhar','gram']
        print(crops1[index])
        name=crops1[index]
        ...
        return render_template('predictcrop.html', context=context)
